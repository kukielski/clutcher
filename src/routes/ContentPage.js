...
export default function ContentPage() {
    ...
    async function fetchAll() {
      try {
        ...
        const lessonFetches = [
          ...
          ...teams.map(team => {
            console.log('Fetching lessons for team:', team.label, team.id);
  ...
  // Sort: "No Team" first, then by team label, then lesson name
  ...
  // Filter lessons based on search input (case-insensitive)
  ...
  return (
    <div className="contents-container">
      <h1 style={{ marginBottom:0 }}>Lessons by Team</h1>
      <p style={{ margin: "0.5em" }}>{HOST}</p>
      <input
